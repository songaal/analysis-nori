...
public class SimpleAnalysisTest extends LuceneTestCase  {
	...
	@Test public void longTextTest() throws Exception {
		...
		final String text = "아름다운ABC이땅에123금수강산에a12단군할아버지가터잡으시고홍익인간뜻으로나라세우니대대손손훌륭한인물도많아고구려세운동명왕백제온조왕알에서나온혁거세";

        final Settings settings = Settings.builder().put("index.analysis.analyzer.my_analyzer.type", "nori")
    ...
    public void createDictionaryTest() {
        ...
        try {
            ...
            File csvFile1 = new File(inputDir, "dict1.csv");
            writer = new FileWriter(csvFile1);
            //writer.write("한국어,0,0,0,NNG,,,,*,*,*,0");
            writer.write("대우,,,,NNP,*,F,대우,*,*,*,*");
            writer.close();
    ...
    public Analyzer createTestAnalysis(Settings analysisSettings) throws IOException {
        ...
        final AnalysisNoriPlugin plugins = new AnalysisNoriPlugin();
